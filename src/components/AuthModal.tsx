...
import { Gift, LogIn, UserPlus, Eye, EyeOff, Loader2, X } from 'lucide-react';
...
export default function AuthModal({ onAuthenticated, addToast, onClose }: AuthModalProps) {
  ...
  const cardContent = (
    <div className="relative rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white/95 dark:bg-primary-900/95 shadow-2xl overflow-hidden backdrop-blur-sm">
      {onClose && (
        <button
          onClick={onClose}
          className="absolute top-3.5 right-3.5 z-10 rounded-xl p-1.5 text-zinc-400 hover:text-zinc-600 hover:bg-primary-100 dark:text-zinc-500 dark:hover:text-zinc-300 dark:hover:bg-primary-800 transition-colors"
          title="Close"
        >
          <X className="h-5 w-5" />
        </button>
