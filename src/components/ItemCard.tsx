...
export default function ItemCard({
  ...
  // Whether we need to show action buttons at the bottom on mobile
  const hasActions = isOwner || (!isOwner && !isClaimed) || (!isOwner && isClaimedByMe);
  ...
  return (
    ...
    >
      <div className="flex flex-col sm:flex-row">
          ...
          <div className="flex-1 px-3 sm:px-4 py-3 flex flex-col justify-between min-w-0">
            <div>
              ...
              <div className="flex items-baseline gap-2">
                <h3 className="text-base font-semibold text-black dark:text-white line-clamp-2 sm:truncate">{item.title || 'Untitled item'}</h3>
                {item.price && (
        ...
        {/* Action Buttons — below content on mobile, inline on sm+ */}
        {hasActions && (
          <div className="flex items-center gap-1 shrink-0 sm:px-3 border-t sm:border-t-0 border-zinc-200 dark:border-zinc-800">
            {isOwner && (
              <div className="flex items-center gap-1 px-3 py-2 sm:py-0 sm:px-0">
                <button
                  onClick={() => onEdit?.(item)}
            ...
            {!isOwner && !isClaimed && (
              <div className="flex-1 sm:flex-none px-3 py-2 sm:py-0 sm:px-0">
                <button
                  onClick={() => onClaim?.(item)}
                  className="w-full sm:w-auto flex items-center justify-center gap-1.5 rounded-xl bg-primary-800 px-3.5 py-2 text-xs font-semibold text-primary-50 shadow-sm hover:bg-primary-900 dark:bg-primary-200 dark:text-primary-900 dark:hover:bg-primary-300 transition-colors"
                >
            ...
            {!isOwner && isClaimedByMe && (
              <div className="flex-1 sm:flex-none px-3 py-2 sm:py-0 sm:px-0">
                <button
                  ...
                  className="w-full sm:w-auto flex items-center justify-center gap-1.5 rounded-xl border border-primary-300 bg-primary-50 px-3.5 py-2 text-xs font-semibold text-zinc-700 hover:bg-primary-100 dark:border-zinc-800 dark:bg-primary-800 dark:text-primary-200 dark:hover:bg-primary-700 transition-colors"
                >
                  Unclaim
                </button>
              </div>
